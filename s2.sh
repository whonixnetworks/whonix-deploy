...
# Whonix Initium Script for Debian/Ubuntu, Fedora, and Alpine
...
check_system() {
    ...
    if command -v apt-get &>/dev/null; then
        ...
        echo -e "${GREEN}System compatible: $distro $version (using apt)${RESET}"
    elif command -v dnf &>/dev/null; then
        package_manager="dnf"
        distro=$(grep -E '^ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"' | tr '[:upper:]' '[:lower:]')
        version=$(grep -E '^VERSION_ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"')
        echo -e "${GREEN}System compatible: $distro $version (using dnf)${RESET}"
    elif command -v apk &>/dev/null; then
        package_manager="apk"
        distro=$(grep -E '^ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"' | tr '[:upper:]' '[:lower:]')
        version=$(grep -E '^VERSION_ID=' /etc/os-release | cut -d'=' -f2 | tr -d '"')
        echo -e "${GREEN}System compatible: $distro $version (using apk)${RESET}"
    else
...
initial_setup() {
    ...
    echo -e "${YELLOW}For Debian/Ubuntu, Fedora, and Alpine${RESET}"
    echo -e "${YELLOW}==============================================${RESET}\n"
...
update_install() {
    ...
    case "$package_manager" in
        apt)
            ...
            ;;.*
            (sudo dnf check-update -y >/dev/null 2>&1) & spinner $!
            ;;.*
            (sudo apk update >/dev/null 2>&1) & spinner $!
            ;;.*
    esac
    ...
    case "$package_manager" in
        apt)
            packages_apt=(btop wireguard wireguard-tools speedtest-cli bc unattended-upgrades software-properties-common tmux snapd neofetch mc htop iotop iftop wget curl jq nano git coreutils rclone rsync python3 python3-pip figlet p7zip-full docker.io docker-compose-v2 wipe ufw openssh-server ipcalc)
            (sudo DEBIAN_FRONTEND=noninteractive apt-get install -y "${packages_apt[@]}" >/dev/null 2>&1) & spinner $!
            ;;.*
            packages_dnf=(btop wireguard-tools speedtest-cli bc tmux snapd neofetch mc htop iotop iftop wget curl jq nano git coreutils rclone rsync python3 python3-pip figlet p7zip-full docker docker-compose wipe ufw openssh-server ipcalc)
            (sudo dnf install -y "${packages_dnf[@]}" >/dev/null 2>&1) & spinner $!
            ;;.*
            packages_apk=(btop wireguard-tools speedtest-cli bc tmux snapd neofetch mc htop iotop iftop wget curl jq nano git coreutils rclone rsync python3 py3-pip figlet p7zip docker docker-compose wipe ufw openssh ipcalc)
            (sudo apk add --no-cache "${packages_apk[@]}" >/dev/null 2>&1) & spinner $!
            ;;.*
    esac
    ...
    case "$package_manager" in
        apt)
            ...
            ;;.*
            (sudo dnf upgrade -y >/dev/null 2>&1) & spinner $!
            (sudo dnf autoremove -y >/dev/null 2>&1) & spinner $!
            ;;.*
            (sudo apk upgrade >/dev/null 2>&1) & spinner $!
            ;;.*
    esac
    ...
    (
        ...
        case "$package_manager" in
            apt)
                ...
                ;;.*
                add_alias "update" "sudo dnf check-update && sudo dnf upgrade -y"
                add_alias "install" "sudo dnf install -y"
                add_alias "clean" "sudo dnf autoremove -y"
                ;;.*
                add_alias "update" "sudo apk update && sudo apk upgrade"
                add_alias "install" "sudo apk add"
                add_alias "clean" "sudo apk del"
                ;;.*
        esac
        add_alias "c" "clear"
        add_alias "h" "history"
